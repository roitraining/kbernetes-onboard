# Open the frontend yaml in code editor again and add the following (aligned to the left) at the end of the file, including the ---
...
# It's time to test our autoscaling. C reate a compute engine vm to send lots of requests from:
...
# Go to compute engine in the console an duse the button to ssh into the machine. Once there, execute the following to update apt-get and install apache bench. (You will be asked to confirm the install of apache utils)
...
# Return to cloud shell and run get pods several times. It will take quite a while - maybe 30 secs, but you should eventually  see that the scaler has increased the number of pods. Note the delay between running and ready - our readiness probe.
